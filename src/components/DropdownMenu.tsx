...
export function Dropdown({ options, placeholder }: DropdownProps) {
	...
	return (
		<div className="relative w-64">
			{/* Dropdown Button */}
			<button onClick={toggleDropdown} className="w-full flex items-center justify-between border rounded-lg px-4 py-2 font-body text-gray-800 focus:outline-none shadow-sm">
				<span>{selected || placeholder}</span>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					className={`w-4 h-4 transform transition-transform ${isOpen ? 'rotate-180' : ''} text-red-500`}
					viewBox="0 0 20 20"
					fill="red"
				>
					<path
